    ...
    const submit = async (data) => {
        if (stage === 0) {
            ...
            try {
                ...
                const response = await axios.post(`${process.env.BACKEND_URL}/stripe/create-payment-intent-item`, postData, {withCredentials: true});
                ...
                console.log('Failed to create payment intent item: ', error);
            }
        } else if (stage === 1) {
            const response = stripeClient.confirmCardPayment(paymentIntent.client_secret, {
                ...
                router.push(`${process.env.DOMAIN_URL}/checkout/item?paymentIntentId=${response.paymentIntent.id}&id=${item._id}&emailAddress=${enteredEmail}&username=${username}`)
            }).catch((error) => {
                console.log(`Failed to confirm payment intent item: ${response.error.message}`)
            })
        }
    }
